    ...
    ui(new Ui::MainWindow)
{
    ...
    this->ui->customPlot->yAxis->setLabel("Pitch (b) & Temp(r) (deg)");
    QSharedPointer<QCPAxisTickerTime> timeTicker(new QCPAxisTickerTime);
    timeTicker->setTimeFormat("%h:%m:%s");
    this->ui->customPlot->xAxis->setTicker(timeTicker);
    this->ui->customPlot->yAxis->setRange(-50,550);
    this->ui->customPlot->replot();
    this->ui->outputText->setPlainText(TOPIC);
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    QJsonDocument doc = QJsonDocument::fromJson(str.toUtf8());
        ...
        QJsonObject data = obj["d"].toObject();
        Pitch = (float)  data["Pitch"].toDouble();
        Temp = (float) data["CPU Temperature"].toDouble();

        // create the output message
        QString outputMsg = QString("topic: ee513/TTempPitch\nmessage: {\"d\":{\"Pitch\": %f,\Temp\": %f }}\nPitch: %1\nTemp : %2")
                                .arg(Pitch).arg(Temp);
